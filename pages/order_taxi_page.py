...
class Tariffs(BasePage):
    ...
    def get_all_titles(self):
        # Ждем, пока все элементы будут видимыми и будут иметь текст
        elements = self.find_all(TariffTaxiLocators.ALL_TITLES)
        # Ждем, пока все элементы будут иметь непустой текст
        self.wait.until(lambda d: all(
            el.text.strip() for el in d.find_elements(*TariffTaxiLocators.ALL_TITLES)
        ))
        # Получаем текст еще раз после ожидания
        elements = self.driver.find_elements(*TariffTaxiLocators.ALL_TITLES)
    ...
    def get_selected_tariff_price(self):
        """Возвращает цену активного тарифа."""
        price_text = self.find(TariffTaxiLocators.ACTIVE_PRICE).text.strip()
        import re
