...
class DriveActions(BasePage):
    ...
    def is_book_drive_enabled(self):
        try:
            # Используем find_present для поиска элемента в DOM (не обязательно видимого)
            element = self.find_present(DriveLocators.BOOK_BUTTON)
            # Проверяем, что элемент видимый и активный
            if not element.is_displayed():
                return False
            # Проверяем через is_enabled из BasePage
            return self.is_enabled(element)
        except TimeoutException:
            return False
